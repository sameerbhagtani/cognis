...
| 'folder'           | '(workspaceId, deletedAt)' composite | Trash view; also serves plain 'workspaceId' lookups via leftmost-prefix, so no separate 'workspaceId' index is needed           | 'SELECT * FROM folder WHERE workspaceId = $1 AND deletedAt IS NOT NULL'    |
...
| 'note'             | '(workspaceId, deletedAt)' composite | Trash view; also serves plain 'workspaceId' lookups via leftmost-prefix, so no separate 'workspaceId' index is needed           | 'SELECT * FROM note WHERE workspaceId = $1 AND deletedAt IS NOT NULL'      |
